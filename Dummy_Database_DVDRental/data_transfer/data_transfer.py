...
# create a logger with custom name, formatter and handler
...
class fetcher_and_exporter(configuration.Credential):
        ...
        def fetch_export(self, table_name):
                ...
                try:
                        ...
                        logger.info(f'{table_name}.csv successfully created')
                        return True
                except Error as e:
                        print(f"Error: {e}")
                        logger.error(f"Error: {e} while fetching data from {table_name} table")
                        return False
        ...
        def insert_data(self, source_table, target_table):
                ...
                source_database = self.load_env_var()['source_database']
                target_database = self.load_env_var()['target_database']
                try:
                        ...
                        return True
                            ...
                            print(f"Error: {e}")
                            logger.error(f"{source_database} - {source_table} - {target_database} - {target_table} - {e}")
                            return False
